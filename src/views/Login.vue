<template>
  <v-snackbar transition=" slide-y-reverse-transition" variant="text" text color="primary" v-model="error.val">
  ...
  <v-row style="height:100%" align="center" justify="center">
    <v-col cols="11" sm="6" md="4" lg="3" xl="2">
      ...
      <v-sheet class="pa-3">
        <v-btn @click="handleLogin(method.name ?? method)" class="mb-2" v-for="(method, i) in methods.primary" :key="i"
          :rounded="0" size="large" color="black" :prepend-icon="`mdi-${method.icon ?? method.toLowerCase()}`" block
          flat>
          {{ method.name ?? method }}
        </v-btn>
